...
func TestGetCurrentUserQuery_Execute(t *testing.T) {
	...
	query := NewGetCurrentUserQuery(repo)
	...
	t.Run("User Not Found", func(t *testing.T) {
		...
		if appErr, ok := err.(*pkgerrors.AppError); ok {
			if appErr.Code != pkgerrors.CodeUserNotFound {
				t.Errorf("Error code = %v, want %v", appErr.Code, pkgerrors.CodeUserNotFound)
			}
		} else {
